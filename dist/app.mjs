...
function updateButtons(){const s=setting(),bounds=stickerBounds(s),valid=!!image&&bounds.inside&&bounds.area<=.35&&!!s.text;
 ...
 $('next-level').disabled=busy||!progress[index]||index===LEVELS.length-1;
...
function defaults(){ $('sticker-text').value=level().text;$('sticker-size').value='45';$('sticker-x').value='50';$('sticker-y').value='50';$('sticker-angle').value='0';}
async function selectLevel(i){if(busy||loading||i<0||i>=LEVELS.length||(i>0&&!progress[i-1]))return;index=i;version++;const token=++levelGeneration;baseline=null;lastRun=null;control=null;image=null;defaults();$('level-number').textContent=String(i+1).padStart(2,'0');$('level-title').textContent=level().title;$('mission').textContent=`让模型把${NAMES[level().source]}排在后面，把${NAMES[level().target]}排到第一。`;$('target-label').textContent=NAMES[level().target];$('hint-text').textContent=level().hint;$('hint-text').hidden=true;
...
$('load-model').onclick=start;$('cancel-load').onclick=()=>stopWorker('已取消下载，可重新启动。');$('run').onclick=()=>submit(false);$('blank').onclick=()=>submit(true);$('reset-sticker').onclick=()=>{defaults();changed();};$('next-level').onclick=()=>selectLevel(index+1);$('hint').onclick=()=>{$('hint-text').hidden=!$('hint-text').hidden;};
